<div class="bg-white pb-4 mx-2 sm:mx-0 flex items-center flex-wrap">
    <ul class="flex items-center bg-gray-100 rounded-md px-2">
        ...
        @isset($product->productCategories)
            <li class="inline-flex items-center">
                <a href="{{ route('category.detail', $product->productCategories->id) }}" class="text-xs font-medium md:text-sm text-gray-600 hover:text-red-500">
                    {{ $product -> productCategories -> name }}
                </a>
                <span class="mx-2 h-auto text-gray-400 font-medium mb-1"> <i class="fa-solid fa-chevron-right text-xs"></i> </span>
            </li>
        @endisset
        <li class="inline-flex items-center">
            <p class="text-xs md:text-sm text-gray-600 text-blue-500 line-clamp-2">
                {{ $product -> name }}
            </p>
        </li>
    </ul>
