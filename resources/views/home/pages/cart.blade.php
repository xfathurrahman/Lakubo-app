...
@section('content')
    <div class="mb-14" id="refreshcart">
        @foreach($userCarts as $store)
            <div class="w-full mx-auto bg-gray-100 shadow-lg rounded-lg mt-6">
            <div class="md:flex ">
                <div class="w-full p-4 px-5 py-5">
                        <div class="flex justify-between border-b pb-5">
                            <div class="font-medium text-lg">Pesanan {{ $loop->iteration }} dari <p class="text-red-400 inline">{{ $store->products->stores->name }}</p></div>
                            <div class="font-medium text-lg">{{ $userCarts->count() }} Item</div>
                        </div>
                        @if ( $userCarts->count() > 0 )
                            ...
                            @php $total = 0 @endphp
                            @foreach($userCarts as $cart_item)
                                <div class="product_data flex items-center hover:bg-gray-200 px-6 py-5">
                                    <div class="flex grid grid-cols-4 w-2/5">
                                        ...
                                        <div class="col-span-4 lg:col-span-3 flex flex-col justify-between text-center lg:text-left px-2 mt-2 lg:ml-4 lg:mt-0 flex-grow">
                                            ...
                                            <a href="#" class="del-cart-btn text-center bg-red-400 rounded font-semibold text-white text-xs">Hapus</a>
                                        </div>
                                    </div>
